...
export default function WebToPDFConverter({ onPDFGenerated }: WebToPDFConverterProps) {
  ...
  React.useEffect(() => {
    console.log('Images state changed:', images, 'Length:', images.length);
  }, [images]);

  // Debug logging for content and CSS changes
  React.useEffect(() => {
    console.log('Structured content changed:', structuredContent ? 'Content available' : 'No content');
    console.log('Custom CSS changed:', customCSS ? 'CSS available' : 'No CSS');
    // Increment key to force re-render
    setContentUpdateKey(prev => prev + 1);
  }, [structuredContent, customCSS]);

  // Generate a unique CSS class for this component instance
  const previewId = React.useMemo(() => `preview-${Math.random().toString(36).substr(2, 9)}`, []);

  // Cleanup function for dynamic styles
  React.useEffect(() => {
    return () => {
      // Cleanup any dynamic styles created by this component
      const dynamicStyles = document.querySelectorAll(`style[data-scope="${previewId}"]`);
  ...
  const fetchWebContent = async () => {
    ...
    try {
      ...
      console.log('Images from result:', result.metadata?.images);
      console.log('Images length:', result.metadata?.images?.length || 0);
      console.log('Full metadata:', result.metadata);
      
      setFetchedContent(result.content);
      const extractedImages = result.metadata?.images || [];
      console.log('Setting images state to:', extractedImages);
  ...
  const processContentWithAI = async () => {
    ...
    try {
      ...
      setStructuredContent(structuredHtml as string);
      ...
      const typedResult = typesettingResult as TypographyResult;
      setStructuredContent(typedResult.formattedContent);
      setCustomCSS(typedResult.styling.css);
      setSuggestions(typedResult.suggestions);
      // Force preview update
      setContentUpdateKey(prev => prev + 1);
    ...
    try {
      ...
      try {
        ...
        if (customCSS) {
          ...
          tempElement.classList.add('temp-pdf-element');
        }
        ...
        try {
          ...
          if (customCSS) {
            ...
            styleSheets.forEach(sheet => {
              if (sheet.textContent?.includes('.temp-pdf-element')) {
  ...
  ): Promise<string | TypographyResult | RefinementResult> => {
    return new Promise((resolve, reject) => {
      let result: string | TypographyResult | RefinementResult | null = null;
  ...
  const handleRefineContent = async () => {
    ...
    if (!structuredContent && !fetchedContent) {
      setError('No content available to refine. Please generate content first.');
      return;
    }
    ...
    if (!originalContent) {
      setOriginalContent(structuredContent || fetchedContent);
      setOriginalCSS(customCSS);
    }

    try {
      const refinementRequest = {
        currentContent: structuredContent || fetchedContent,
      ...
      const typedRefinement = refinementResult as RefinementResult;
      setStructuredContent(typedRefinement.refinedContent);
      if (typedRefinement.refinedCSS) {
        setCustomCSS(typedRefinement.refinedCSS);
      }
      ...
      setSuggestions([
        ...typedRefinement.suggestions,
        `Applied changes: ${typedRefinement.changes.join(', ')}`
      ]);

      // Add to refinement history
      setRefinementHistory(prev => [...prev, {
        feedback: userFeedback,
        changes: typedRefinement.changes,
        timestamp: new Date()
      }]);
  ...
  const resetToOriginal = () => {
    if (originalContent) {
      setStructuredContent(originalContent);
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/80 backdrop-blur-md rounded-xl p-6 border border-gray-200">
              ...
              <select
                id="documentType"
                value={documentType}
                onChange={(e) => setDocumentType(e.target.value as 'academic' | 'business' | 'newsletter' | 'report' | 'article' | 'calendar')}
          ...
          <div className="space-y-6">
            <div className="bg-white/90 backdrop-blur-md rounded-xl p-6 border border-gray-200">
              ...
              <div 
                ref={previewRef}
                key={`preview-${contentUpdateKey}`}
                className={`${styles.previewContent} ${previewId} border-2 border-gray-300 rounded-lg min-h-[500px] max-h-[700px] overflow-y-auto shadow-inner bg-white`}
                style={{ pageBreakInside: 'avoid' }}
              >
                {/* Apply custom CSS scoped to this preview container only */}
                {customCSS && (
                  <style 
                    data-scope={previewId}
                    dangerouslySetInnerHTML={{
                      __html: `
                        ...
                        .${previewId} * {
                          /* Reset some properties to prevent inheritance conflicts */
                          box-sizing: border-box;
                        }
                        /* Scope all selectors within the custom CSS */
                        ${customCSS.replace(/([^{}]+){/g, (match, selector) => {
                          ...
                          if (trimmedSelector.includes(`.${previewId}`)) {
                            return match;
                          }
                          // Scope to preview container
                          return `.${previewId} ${trimmedSelector} {`;
                        })}
                      `
                    }}
                  />
